...
//            add_action('atmf_hidden_data_show', array($this , 'hidden_data_show') );
        ...
        public function get_all_post($metadata, $post_type, $categories, $content_limit)
        {
            ...
            foreach ($query as $key => $post) {
                ...
                // ver que onda $content = apply_filters ("the_content", $post->post_content);
                $content = $post->post_content;

                $data['post_content'] = $this->get_post_content($content, $post_link, $content_limit, $post_custom_link, $post->ID);

                //$data['post_content'] = $content;


                $data['post_title'] = $this->get_title($post->post_title, $post_link, $new_tab);
                $data['post_permalink'] = $post_link;
                $data['post_date'] = $post->post_date;
                $data['comment_count'] = intval($post->comment_count);
                $data['post_thumbnail'] = $this->get_thumbnail(catch_that_image_by_post($post,'small'), $post_link, $new_tab);
      ...
      //      $file = plugin_dir_path( __FILE__ ) . 'templates/' . get_post_meta( $post->ID, '_wp_page_template', true );
        ...
        public function get_post_content($postContent, $href, $limit, $post_custom_link, $post_id)
        {
            ...
            if ($post_custom_link) {
                ...
                $link_ext = strtoupper(substr($href, -3));
                if ($link_ext === "PDF") {
                    $btn_caption = "Open PDF file";
                } else {
                    $btn_caption = "Go to website";
                }

                $content = "<p>" . $postContent . "</p>";
                $content .=  '<a href="'. $href . '" class="hollow button float-right" target="_blank">'.$btn_caption.'</a>';
         ...
         * OE Load readmore.
         */
        public function get_title($title, $href, $new_tab)
        {
            if ($this->has_readmore) {
                $title = '<a href="'. $href . '"' . $new_tab.'>'.$title.'</a>';
            }
            return $title;
        }
         ...
         * OE Load readmore.
         */
        public function get_thumbnail($img, $href, $new_tab)
        {
            if ($this->has_readmore) {
                $img = '<a href="'. $href . '"' . $new_tab.'><img class="thumbnail" src="' . $img . '"/></a>';
            } else {
